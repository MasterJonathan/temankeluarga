...
class MemoriesPage extends ConsumerWidget {
  ...
  void _showWriteDiaryDialog(BuildContext context, WidgetRef ref) {
    ...
    showModalBottomSheet(
      context: context,
      isScrollControlled: true, // Wajib agar bisa full screen/resize saat keyboard muncul
      useSafeArea: true,        // SOLUSI: Menghindari tertutup tombol Back/Home Android
      backgroundColor: AppColors.surface,
                ...
                ElevatedButton(
                  onPressed: () {
                    if (textController.text.isNotEmpty) {
                      ref.read(galleryControllerProvider.notifier).postDiary(
                        textController.text, 
                        null
                      );
                      Navigator.pop(ctx);
...
class _DailyGroupItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IntrinsicHeight( // Agar tinggi garis mengikuti tinggi konten
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // 1. TIMELINE KIRI
          SizedBox(
            ...
            child: Column(
              children: [
                const SizedBox(height: 20), // Padding agar sejajar dengan jam post pertama
                Text(
                  parts[0], // Tanggal
                ...
                Expanded(
                  child: Container(
                    ...
                    color: AppColors.textSecondary.withOpacity(0.3), // Warna lebih samar
                  ),
                ),
              ],
            ),
          ...
          Expanded(
            child: Column(
              children: posts.map((post) => _DiaryPostItem(post: post)).toList(),
            ),
          ),
        ],
      ),
          ...
          Row(
            children: [
              InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: post.selectedReaction != null ? AppColors.secondary.withOpacity(0.2) : Colors.transparent,
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(
                  ...
                  child: Row(
                    children: [
                      Icon(
                        post.selectedReaction != null ? Icons.emoji_emotions : Icons.add_reaction_outlined,
                        size: 18,
                        color: post.selectedReaction != null ? AppColors.primary : AppColors.textSecondary,
                      ),
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          ...
                          color: post.selectedReaction != null ? AppColors.textPrimary : AppColors.textSecondary
                        )
                      ),
                    ],
                  ),
              ...
              if (post.reactionCounts.isNotEmpty)
                ...post.reactionCounts.entries.map((e) => Padding(
      ...
      builder: (ctx) => Container(
        ...
        child: Row(
          ...
          children: emojis.map((emoji) => GestureDetector(
            onTap: () {
              ref.read(galleryControllerProvider.notifier).reactToPost(postId, emoji);
              Navigator.pop(ctx);
            },
